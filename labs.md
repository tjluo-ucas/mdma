...
1. **Pre-frame**：学生先提交决策者、行动、目标、约束、单位、未知和失败条件；
2. **Attempt first**：核心提示前必须有自己的结构图、公式、代码、工作簿或分析；
3. **Evidence diagnosis**：Tutor区分问题、表示、求解、验证、解释和实施缺口；
4. **Bounded hint**：从追问到方法建议逐级提示，不释放同题完整模型或答案；
5. **Deterministic checks**：检查单位、变量域、约束残差、终止状态、Gap、Seed和日志；
6. **Explain-back**：学生解释 Agent 建议、采纳/拒绝理由和建议如何变化；
7. **Transfer probe**：改变目标、约束、不确定性、组织情境或工具，重新捍卫方案。

<div class="notice"><strong>Codex使用协议</strong>Agent可以生成候选实现、测试和情景，并在受控环境运行；不得猜造缺失业务数据、静默修改约束或伪造Solver结果。无法解释的模型和没有真实运行证据的“最优解”不能通过Decision Gate。</div>
